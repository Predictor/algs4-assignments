...
public class KdTree {
    ...
    private static class KdNode {
        private final Point2D p;    // point created for the node
        private final RectHV r;     // outer rectangle surrounding the node
    ...
    private KdNode insert(KdNode node, Point2D p, double x0, double y0,
        ...
        else if (Double.compare(node.p.x(), p.x()) == 0 && Double.compare(node.p.y(), p.y()) == 0) return node;
    ...
    private boolean contains(KdNode node, Point2D p, boolean xcmp) {
        ...
        else if (Double.compare(node.p.x(), p.x()) == 0 && Double.compare(node.p.y(), p.y()) == 0) return true;
        else {
            // The current node is vertical: compare x-coordinates
            if (xcmp) {
                double cmp = p.x() - node.p.x();
